...
class addToCalendarViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate {
    ...
    let userSavedEvent = 0
    let EventFile = WriteToTextFile(fileNameEntered: "savedEvent.txt")
    
    
    @IBOutlet weak var titlePopupLabel: UILabel!
    @IBOutlet weak var descriptionPopupLabel: UILabel!
    @IBOutlet weak var textDescriptionPopUp: UITextView!
    ...
    override func viewDidLoad() {
        ...
        textDescriptionPopUp.backgroundColor = UIColor.clear
...
//        descriptionPopupLabel.text! = getdescriptionPopup
        textDescriptionPopUp.text! = getdescriptionPopup
    ...
    @IBAction func btnAddingToCalendar(_ sender: Any) {
        ...
        // Feedback de cuantas veces el usuario guarda eventos.
        let annotationSaved = userSavedEvent + 1
        let date = Date()
        
        let stringSendedToFile = "TheDelimeter\n\(date)\n" + String(annotationSaved)
        EventFile.writeToFile("***************Feedback De Evento Guardado****************", stringToFile: stringSendedToFile + "\n")
        ...
        eventStore.requestAccess(to: .event) { (granted, error) in
            ...
            if (granted) && (error == nil)
            {
                ...
                // creando el alert
                let successAlert = UIAlertController(title: "¡Se ha Guardado con Éxito!", message: "El evento se a guardado con éxito en el calendario de su dispositivo.", preferredStyle: UIAlertControllerStyle.alert)
                // Agregando el botón
